                                    ...
                                    <p>Produto: 
                                        <strong>
                                            @foreach ($products as $product)
                                                @if ($mysale->code == $product->code)
                                                {{ $product->name }}</strong> 
                                                @endif
                                            @endforeach
                                            
                                    </p>
                                    <p>Quantidade do produto <strong>{{ $mysale->quantity }}</strong></p>
                                    <p>Mes / Ano: <strong>{{ $mysale->month }}/{{ $mysale->year }}</strong></p>
                                    <p>Preço da compra: <strong>R$ {{ number_format($mysale->price, 2, ',', '.') }} </strong></p>
                                    <p>Data de compra: <strong>{{ date('d/m/Y', strtotime($mysale->due_date)) }}</strong></p>
                                    <p>Data de compra:
                                        <strong>
                                            @if ($mysale->purchase_date != null)
                                                {{ date('d/m/Y', strtotime($mysale->purchase_date)) }}
                                            @else
                                                NC
                                            @endif
                                        </strong>
