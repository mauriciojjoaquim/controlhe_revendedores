<x-layout-app page-title="New Product" color-site-bg="{{ $conf['color_site_bg'] }}" bg-color-table="{{ $conf['bg_color_table'] }}" color-table-text="{{ $conf['color_table_text'] }}" color-card-bg="{{ $conf['color_card_bg'] }}" color-card-text="{{ $conf['color_card_text'] }}" bg-color-menu-vert="{{ $conf['bg_color_menu_vertical'] }}" color-menu-vert-text="{{ $conf['color_menu_vertical_text'] }}" bg-color-menu-hor="{{ $conf['bg_color_menu_horisontal'] }}" color-menu-hor-text="{{ $conf['color_menu_horisontal_text'] }}"  text-color-site="{{ $conf['text_color_site'] }}" op-bg-color-site="{{ $conf['bg_color_site'] }}">
    ...
    <div class="w-100 p-1">
        ...
        <form action="{{ route('adm.products.created-product') }}" method="post" enctype="multipart/form-data">
                ...
                <div class="row gap-3 justify-content-center">
                                    ...
                                    <div class="row cols-2 row-cols-sm-1 row-cols-md-1 row-cols-lg-1">
                                        ...
                                        <div class="col mb-3">
                                            <div class="border {{ $conf['color-border'] }} p-4">
                                                <p class="mb-0">Abilitar/Dezabilitar </p>
                                            <div class="border {{ $conf['color-border'] }}">
                                                <div class="d-flex justify-content-start p-2">
                                                    <div class="me-3">
                                                        @if (old('confirmed') == 'on')
                                                            <input type="checkbox" class="btn-check" name="confirmed" id="confirmed" autocomplete="off" checked>
                                                            <label class="btn btn-outline-primary" for="confirmed">Confirmed</label><br>
                                                        @else
                                                            <input type="checkbox" class="btn-check" name="confirmed" id="confirmed" autocomplete="off">
                                                            <label class="btn btn-outline-primary" for="confirmed">Confirmed</label><br>
                                                        @endif
                                                        </div>
                                                    <div class="me-3">
                                                        @if (old('non_production') == 'on')
                                                            <input type="checkbox" class="btn-check" name="non_production" id="non_production" autocomplete="off" checked>
                                                            <label class="btn btn-outline-primary" for="non_production">Fora de produçao</label>
                                                        @else
                                                            <input type="checkbox" class="btn-check" name="non_production" id="non_production" autocomplete="off">
                                                            <label class="btn btn-outline-primary" for="non_production">Fora de produçao</label>
                                                        @endif
                    ...
                    <div class="col-6">
                        <div class="d-flex justify-content-center">
                            <div class="mt-3">
                                <a href="{{ route('adm.products.table-product') }}" class="btn btn-outline-warning me-3 mb-4">Cancel</a>
