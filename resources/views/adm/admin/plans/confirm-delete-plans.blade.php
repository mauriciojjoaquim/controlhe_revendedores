<x-layout-app page-title="Delete Plano" color-site-bg="{{ $conf['color_site_bg'] }}" bg-color-table="{{ $conf['bg_color_table'] }}" color-table-text="{{ $conf['color_table_text'] }}" color-card-bg="{{ $conf['color_card_bg'] }}" color-card-text="{{ $conf['color_card_text'] }}" bg-color-menu-vert="{{ $conf['bg_color_menu_vertical'] }}" color-menu-vert-text="{{ $conf['color_menu_vertical_text'] }}" bg-color-menu-hor="{{ $conf['bg_color_menu_horisontal'] }}" color-menu-hor-text="{{ $conf['color_menu_horisontal_text'] }}"  text-color-site="{{ $conf['text_color_site'] }}" op-bg-color-site="{{ $conf['bg_color_site'] }}">
        ...
        <div class="row row-cols-1 row-cols-sm-1 row-cols-md-1 row-cols-xl-1">
            ...
            <div class="col-xl-12 col-md-12 col-md-12 col-sm-12">
                ...
                <div class="text-center">
                    <h3 class="my-5">{{ $plan->category }}</h3>
                    <p>category: {{ $plan->category }}</p>
                    <div class="d-flex justify-content-center">
                        <a href="{{ route('adm.plans.table-plans') }}" class="btn btn-warning px-5">Não</a>
                    <form action="{{ route('adm.plans.deleted-plans') }}" method="post" class="px-2">
                        @csrf
