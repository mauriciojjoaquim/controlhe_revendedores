<x-layout-app page-title="Todas Configurações" color-site-bg="{{ $conf['color_site_bg'] }}" bg-color-table="{{ $conf['bg_color_table'] }}" color-table-text="{{ $conf['color_table_text'] }}" color-card-bg="{{ $conf['color_card_bg'] }}" color-card-text="{{ $conf['color_card_text'] }}" bg-color-menu-vert="{{ $conf['bg_color_menu_vertical'] }}" color-menu-vert-text="{{ $conf['color_menu_vertical_text'] }}" bg-color-menu-hor="{{ $conf['bg_color_menu_horisontal'] }}" color-menu-hor-text="{{ $conf['color_menu_horisontal_text'] }}"  text-color-site="{{ $conf['text_color_site'] }}" op-bg-color-site="{{ $conf['bg_color_site'] }}">
   ...
   @endif
        ...
        <div class="text-center my-5">
            ...
            <a href="{{ route('adm.Leaders.leader-seller-setting.add-leader-seller-setting') }}" class="btn btn-primary">Create a new Settings</a>
        </div>
    @else
    <div class="mb-3">
                        ...
                        <td>
                             <div class="d-flex gap-1 justify-content-end">
                                <div class="btn-lg-display d-flex gap-1 justify-content-end">
                                    <a href="{{ route('adm.Leaders.leader-seller-setting.show-leader-seller-setting', ['id' => $setting_detail->id])  }}" class="btn btn-sm btn-outline-warning ms-2"><i class="fas fa-eye me-2"></i>Detalhe</a>
                                    <a href="{{ route('adm.Leaders.leader-seller-setting.edit-leader-seller-setting', ['id' => $setting_detail->id]) }}" class="btn btn-sm btn-outline-primary ms-2"><i class="fa-regular fa-pen-to-square me-2"></i>Editar</a>
                                    {{-- <a href="{{ route('adm.Leaders.leader-seller-setting.conf-delete-leader-seller-setting', ['id' => $setting_detail->id]) }}" class="btn btn-sm btn-outline-danger ms-2"><i class="fa-regular fa-trash-can me-2"></i>Excluir</a> --}}
                                 ...
                                 <div class="btn-group btn-sm-display" role="group" aria-label="action">
                                    <div class="btn-group" role="group">
                                      ...
                                      <ul class="dropdown-menu " aria-labelledby="btnGroupDrop1">
                                        <li><a href="{{ route('adm.Leaders.leader-seller-setting.show-leader-seller-setting', ['id' => $setting_detail->id]) }}" class="dropdown-item"><i class="fas fa-eye me-2"></i>Detalhe</a></li>
                                        <li><a href="{{ route('adm.Leaders.leader-seller-setting.edit-leader-seller-setting', ['id' => $setting_detail->id]) }}" class="dropdown-item"><i class="fa-regular fa-pen-to-square me-2"></i>Editar</a></li>
                                        {{-- <li><a href="{{ route('adm.Leaders.leader-seller-setting.conf-delete-leader-seller-setting', ['id' => $setting_detail->id]) }}" class="dropdown-item"><i class="fa-regular fa-trash-can me-2"></i>Excluir</a></li> --}}
                                      </ul>
                                    </div>
                                  </div>
                             </div>
