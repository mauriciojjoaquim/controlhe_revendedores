<x-layout-app page-title="New magazine cycle" color-site-bg="{{ $conf['color_site_bg'] }}" bg-color-table="{{ $conf['bg_color_table'] }}" color-table-text="{{ $conf['color_table_text'] }}" color-card-bg="{{ $conf['color_card_bg'] }}" color-card-text="{{ $conf['color_card_text'] }}" bg-color-menu-vert="{{ $conf['bg_color_menu_vertical'] }}" color-menu-vert-text="{{ $conf['color_menu_vertical_text'] }}" bg-color-menu-hor="{{ $conf['bg_color_menu_horisontal'] }}" color-menu-hor-text="{{ $conf['color_menu_horisontal_text'] }}"  text-color-site="{{ $conf['text_color_site'] }}" op-bg-color-site="{{ $conf['bg_color_site'] }}">

    <div class="w-100 p-4">

        <h3>New magazine cycle</h3>

        <hr>
        {{-- disabled --}}
        <form action="{{ route('adm.dealers.client-products.created-magazine-numbers') }}" method="post">
            ...
            <div class="container-fluid">
                                ...
                                <div class="mb-3">
                                        ...
                                        <select class="form-select" id="supplier_id" name="supplier_id">
                                            ...
                                            @foreach ($suppliers as $supplier)
                                                @if ($supplier->id == old('supplier_id'))
                ...
                <div class="mt-3 d-flex justify-content-center">
                    ...
                    <button type="submit" class="btn btn-outline-primary">New magazine cycle</button>
                </div>
