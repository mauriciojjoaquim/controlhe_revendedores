...
class RellerMyProductsController extends Controller
{
    //Product Table
    public function tableProductSale(): View
    {
        ...
        $products = Product::with('supplier', 'category')->where('user_id', '=', Auth::user()->id)->get();
        // Cofigurações do app do vendedor
        $id_user = Auth::user()->id;
        $conf = SettingsDetail::where('user_id', '=', $id_user)->first();

        return view('adm.resellers.reseller-products.reseller-my-product.table-reseller-my-products', compact('conf', 'products'));
    ...
    public function addProductSale(): View
    {
        ...
        return view('adm.resellers.reseller-products.reseller-my-product.add-reseller-my-products', compact('conf', 'suppliers', 'categories'));
    }
    ...
    public function editProductSale($id): View
    {
        ...
        $product = Product::with('supplier', 'category')->where('user_id', '=', Auth::user()->id)->findOrFail($id);
        // Cofigurações do app do vendedor
        $id_user = Auth::user()->id;
        $conf = SettingsDetail::where('user_id', '=', $id_user)->first();


        return view('adm.resellers.reseller-products.reseller-my-product.edit-reseller-my-products', compact('conf', 'product', 'suppliers','categories'));
    ...
    public function createdProductSale(Request $request)
    {
        ...
        if($request->hasFile('imagem')){
           ...
           $product->photo_url = $fileNameToStore;
                ...
                $product->url_voucher =  '150x150.png';
        ...
        return redirect()->route('adm.resellers.reseller-my-products.table-reseller-my-products');
    ...
    public function showProductSale($id): View
    {
        ...
        return view('adm.resellers.reseller-products.reseller-my-product.show-reseller-products', compact('cnfo', 'product'));
    }
    ...
    public function confDeleteProductSale($id): View
    {
        ...
        return view('adm.resellers.reseller-products.reseller-my-product.confirm-delete-reseller-my-products',compact('conf', 'product'));
    }
